...
# --- PANEL PRINCIPAL ---
st.title("🏢 Estimación de Precio de Inmueble")
st.caption("Aplicando Machine Learning.")

st.divider()

# Inicializar variable de estado para controlar la vista
if 'calculado' not in st.session_state:
    st.session_state.calculado = False

# Si se presiona el botón, activamos la bandera en session_state
if btn_predict:
    st.session_state.calculado = True

if modelo is None:
    st.error(f"⚠️ No se encontró el archivo del modelo ('{MODEL_PATH}').")
elif st.session_state.calculado:
    data_dict = {
        'BEDROOMS': bedrooms,
        'BATHROOMS': bathrooms,
    ...
    try:
        ...
        with tab_map:
            # Crear mapa centrado en las coordenadas ingresadas
            ...
            folium.Marker(
                [lat, lon],
                popup=f"Precio Est.: ${prediccion:,.2f}",
                tooltip="Ubicación del inmueble",
